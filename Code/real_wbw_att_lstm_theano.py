...
def init_wbw_att_layer(params, nin, ndim, options, prefix='wbw_attention'):
    ...
    params[prefix + '_w_r'] = init_weight(nin, ndim, options)
    params[prefix + '_w_alpha'] = init_weight(ndim, 1, options)
    params[prefix + '_w_t'] = init_weight(nin, nin, options)
    return params
...
def wbw_attention_layer(shared_params, image, question, mask, r_0, options, prefix='wbw_attention'):
    ...
    :param image: batch_size x num_regions(196) x n_image_feat(512)
    :param question : T x batch_size x n_embed
    :param r_0 : batch_size x n_image_feat 
    :param mask: mask for x, T x batch_size
    '''
    n_dim = options['n_dim']
    n_image_feat = options['n_image_feat']
    wbw_w_y = shared_params[prefix + '_w_y'] # n_image_feat x n_embed
    wbw_w_h = shared_params[prefix + '_w_h'] # n_embed x n_embed
    wbw_w_r = shared_params[prefix + '_w_r'] # n_image_feat x n_embed
    wbw_w_alpha = shared_params[prefix + '_w_alpha'] # n_embed x 1
    wbw_w_t = shared_params[prefix + '_w_t'] # n_image_feat x n_image_feat
    Y = image
    def recurrent(h_t, mask_t, r_tm1):
        # h_t : bt_sz x n_embed
        wht = T.dot(h_t, wbw_w_h) # bt_sz x n_embed
        # r_tm1 : bt_sz x n_image_feat
        wrtm1 = T.dot(r_tm1, wbw_w_r) # bt_sz x n_embed
        tmp = (wht + wrtm1)[:,None,:] # bt_sz x num_regions x n_embed
        Mt = tanh(T.dot(Y, wbw_w_y) + tmp ) # bt_sz x num_regions x n_embed         
        
        WMt = T.dot(Mt, wbw_w_alpha).flatten(2) # bt_sz x num_regions
        alpha_t = T.nnet.softmax(WMt).dimshuffle((0,'x',1)) # bt_sz x 1 x num_regions
        Y_alpha_t = T.batched_dot(alpha_t, Y)[:,0,:] # bt_sz x n_image_feat   
        r_t = Y_alpha_t + T.dot(r_tm1, wbw_w_t)
...
def build_model(shared_params, options):
    ...
    r_0 = theano.shared(numpy.zeros((batch_size, n_image_feat), dtype='float32'))
    
    r = wbw_attention_layer(shared_params, image_feat, h_encode, input_mask, r_0, options, prefix='wbw_attention')
